...
def create_optimizer(loss, learning_rate, num_train_steps, weight_decay_rate=0.0, use_tpu=False,
    ...
    if layerwise_lr_decay_power > 0:
        learning_rate, key_to_vars = _get_layer_lrs(learning_rate, layerwise_lr_decay_power, n_transformer_layers)
    optimizer = AdamWeightDecayOptimizer(
        learning_rate=learning_rate,
        weight_decay_rate=weight_decay_rate,
        key_to_vars=key_to_vars,
        beta_1=0.9,
        beta_2=0.999,
        epsilon=1e-6,
...
class AdamWeightDecayOptimizer(tf.train.Optimizer):
    ...
    def _apply_gradients(self, grads_and_vars, learning_rate):
        ...
        for (grad, param) in grads_and_vars:
            ...
            m = slot_creator.create_zeros_slot(param, name=six.ensure_str(param_name) + "/adam_m")
            v = slot_creator.create_zeros_slot(param, name=six.ensure_str(param_name) + "/adam_v")
            ...
            for key in self.key_to_vars:
                if key in param_name:
                    l_r = self.learning_rate[key]
            update_with_lr = l_r * update
