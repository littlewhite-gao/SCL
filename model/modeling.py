...
class BertModel(object):
    ...
    model = modeling.BertModel(config=config, is_training=True,
      input_ids=input_idss, input_mask=input_mask, token_type_ids=token_type_ids)
    ...
    def __init__(self,
        ...
        if input_reprs is None:
            if input_embeddings is None:
                with tf.variable_scope(
                      (scope if untied_embeddings else pretrain_model_name) + "/embeddings",
                      reuse=tf.AUTO_REUSE):
            ...
            # utils.heading(self.token_embeddings)
        ...
        # utils.heading(self.embedding_output)
        with tf.variable_scope(scope, default_name=pretrain_model_name, reuse=tf.AUTO_REUSE):
            if pretrain_model_name == 'electra':
                if self.embedding_output.shape[-1] != bert_config.hidden_size:
                  utils.heading('embedding_output.shape[-1] != bert_config.hidden_size')
    ...
    def get_pooled_output(self):
        if self.pooling == 'average':
            utils.heading('-------using average pooling---------')
            self.pooled_output = tf.reduce_mean(self.sequence_output, axis=1)

        elif self.pooling == 'last2_cls_average':
            utils.heading('-------using last2 cls average pooling---------')
            last1_cls = self.sequence_output[:, 0, :]
            last2_cls = self.all_layer_outputs[-2][:, 0, :]
            self.pooled_output = tf.reduce_mean([self.cls_weight * last1_cls, last2_cls], axis=0)
        elif self.pooling == 'last2_pool_cls':
            utils.heading('-------last pool cls and last 2nd cls average---------')
            last1_cls = self.pooled_output
            last2_cls = self.all_layer_outputs[-2][:, 0, :]
            self.pooled_output = tf.reduce_mean([last1_cls, last2_cls], axis=0)
        elif self.pooling == 'last2_seq_average':
            utils.heading('-------using last2 seq average pooling---------')
            average_seq_pooled_output = tf.reduce_mean([self.all_layer_outputs[-2], self.sequence_output], axis=0)
            self.pooled_output = tf.reduce_mean(average_seq_pooled_output, axis=1)
        else:
            utils.heading('-------using first cls pooling ---------')
    ...
    def get_sentence_out(self):
        first_layer_out = self.all_layer_outputs[0]
        first_pooled_output = tf.reduce_mean(first_layer_out, axis=1)
        last_pooled_output = tf.reduce_mean(self.sequence_output, axis=1)
        sentence_out = tf.reduce_mean([first_pooled_output, last_pooled_output], axis=0)
        return sentence_out
...
def embedding_postprocessor(input_tensor,
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            ...
            position_embeddings = tf.reshape(position_embeddings,
                                             position_broadcast_shape)
            if use_cl and c_type == 'shuffle':
                num = int(cut_rate)
                factor = random.sample(range(seq_length - num), 1)
                factor = np.add(factor * num, range(num))
                rows = tf.gather(position_embeddings, factor, axis=0)
                random.shuffle(factor)
                factor = np.reshape(factor, [num, 1])
                position_embeddings = tf.tensor_scatter_nd_update(position_embeddings, factor, rows)
            output += position_embeddings
            # utils.heading(output)
    ...
    if use_cl:
        if c_type == '2dropout':
            pass
        elif c_type == 'drop_embedding':
            output = tf.nn.dropout(output, keep_prob=1-drop_rate)
        elif c_type == 'feature_cutoff':
            cols = np.random.randint(0, width, int(cut_rate*width))
            shadow = [0.0 if i in cols else 1.0 for i in range(width)] * seq_length
            shadow = tf.reshape(shadow, [seq_length, width])
            output *= shadow
        elif c_type == 'token_cutoff':
            rows = np.random.randint(0, seq_length, int(cut_rate * seq_length))
            shadow = [0.0 if i in rows else 1.0 for i in range(seq_length)] * width
            shadow = tf.reshape(shadow, [width, seq_length])
            shadow = tf.transpose(shadow)
            output *= shadow
        elif c_type == 'noise':
            noise_sampler = tf.distributions.Normal(loc=0.0, scale=noise_scale)
            embedding_shape = tf.shape(output)
            noise = noise_sampler.sample(embedding_shape)
            output += noise
        else:
            if c_type != 'shuffle':
                raise ValueError("unknown contrastive type: %s" % c_type)
    return output
