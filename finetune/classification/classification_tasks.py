...
class SingleOutputTask(task.Task):
    ...
    def get_examples(self, split):
        return self._create_examples(read_tsv(
            os.path.join(self.config.data_dir, split + ".tsv"),
...
class RegressionTask(SingleOutputTask):
    ...
    def sim_scl_loss(self, label_ids, pooled_out, temp):
        batch_size = label_ids.shape[0]
        pooled_out = tf.nn.l2_normalize(pooled_out, -1)
        diff = tf.exp(tf.matmul(pooled_out, pooled_out, transpose_b=True) / temp)
        loss = 0.0
        for i in range(2 * batch_size):
            index = i + batch_size if i < batch_size else i -batch_size
            loss += tf.log(tf.pow(diff[i][index], -1) + 1.0)
        return loss

    def get_prediction_module(self, bert_model, features, is_training,
                              percent_done):
        reprs = bert_model.get_pooled_output()
        with tf.variable_scope("targets", reuse=tf.AUTO_REUSE):
            if is_training:
...
class ClassificationTask(SingleOutputTask):
    ...
    def _add_features(self, features, example, log):
        ...
        features[example.task_name + "_label_ids"] = label_id  # int

    def scl_loss(self, label_ids, pooled_out, temp):
        num_labels = len(self._label_list)
        batch_size = label_ids.shape[0]
        one_hot_label_ids = tf.one_hot(label_ids, depth=num_labels)
        one_hot_label_ids = tf.concat([one_hot_label_ids, one_hot_label_ids], axis=0)
        pooled_out = tf.nn.l2_normalize(pooled_out, -1)
        diff = tf.exp(tf.matmul(pooled_out, pooled_out, transpose_b=True) / temp)
        loss = 0.0
        for i in range(2 * batch_size):
            label_index = tf.argmax(one_hot_label_ids[i])
            same_label = one_hot_label_ids[:, label_index]
            coefficient = [0.0 if i == j else 1.0 for j in range(2 * batch_size)]
            neg = tf.reduce_sum(tf.multiply(1 - same_label, diff[i]))
            loss_i = tf.reduce_sum(tf.multiply(same_label * coefficient, tf.log(tf.pow(diff[i], -1) * neg + 1.0)))
            loss += tf.cast(1 / tf.count_nonzero(same_label * coefficient), dtype=tf.float32) * loss_i
        return loss
    ...
    def get_prediction_module(self, bert_model, features, is_training,
        ...
        return losses, outputs, logits, reprs, probs
...
def _truncate_seq_pair(tokens_a, tokens_b, max_length):
    ...
    while True:
        ...
        if len(tokens_a) > len(tokens_b):
            ...
            tokens_b.pop()
    return total_length, len(tokens_a), len(tokens_b)
...
class WNLI(ClassificationTask):
    ...
    def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
        super(WNLI, self).__init__(config, "wnli", tokenizer,
                                   ["0", "1"])
...
class STS(RegressionTask):
    ...
    def _create_examples(self, lines, split):
        examples = []
        if split == "test":
            examples += self._load_glue(lines, split, 5, 6, None, False)
        else:
            examples += self._load_glue(lines, split, 5, 6, 4, False)
        if self.config.double_unordered and split == "train":
            examples += self._load_glue(
                lines, split, 5, 6, 4, True, len(examples), False)
        return examples
